...
class AccountSendInvoice(models.TransientModel):
    ...
    def get_allowed_ids(self):
        ...
        self.allowed_partner_ids = self.env["account.move"].browse(self._context.get('active_ids',self.res_id)).mapped(
                'partner_id').contacts_to_notify()
        return self.allowed_partner_ids
...
class MailThread(models.AbstractModel):
    ...
    def _message_get_default_recipients_on_records(self, records):
        res = super(MailThread, self)._message_get_default_recipients_on_records()
